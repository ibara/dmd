...
void outjmptab(block *b)
{
    ...
    {   targ_size_t targ = def;                     // default
        ...
        if (config.exe & (EX_LINUX64 | EX_FREEBSD64 | EX_OPENBSD64 | EX_DRAGONFLYBSD64 | EX_SOLARIS64 | EX_HAIKU64))
        {
            if (config.flags3 & CFG3pic)
            {
        ...
        else if (config.exe & (EX_LINUX | EX_FREEBSD | EX_OPENBSD | EX_SOLARIS | EX_HAIKU))
        {
            if (config.flags3 & CFG3pic)
            {
...
{
    if (config.exe & (EX_LINUX | EX_FREEBSD | EX_OPENBSD | EX_SOLARIS | EX_HAIKU)) // note: I32 only
    {
        if (config.flags3 & CFG3pic)
        {
